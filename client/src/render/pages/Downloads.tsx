      ...
      {downloadStatus ? (
        <div className="bg-white dark:bg-monokai-dark rounded shadow p-6">
          <div className="flex flex-col">
            ...
            <span>Remaining: {filesQueue - filesDownloaded - filesFailed - filesSkipped} </span>
            <span>Total size: {bytesToFileSize(totalSize)}</span>
            <span>Size of remaining downloads: {bytesToFileSize(totalSize - totalProgress)}</span>
            <span className="font-medium mt-4">Total Progress</span>
            <div className="flex items-center">
              <div className="w-full">
                <LinearProgress variant="determinate" value={calculateTotalProgress()} />
              </div>
              <span className="ml-2">{calculateTotalProgress().toFixed(0)}%</span>
            </div>
            <span className="font-medium mt-4">Current Download</span>
            <div className="flex items-center">
              <div className="w-full">
                <LinearProgress variant="determinate" value={parseFloat(downloadStatus?.currentProgress??"0")} />
              </div>
              <span className="ml-2">{parseInt(downloadStatus?.currentProgress??"0")}%</span>
            </div>
          </div>
        </div>
      ) : (
        <div className="bg-white dark:bg-monokai-dark rounded shadow p-6">
          <div className="flex flex-col gap-2">
            ...
            <Link to="/query">
              <span className="font-medium mt-4 text-blue-500 hover:underline">Go to the beatmap search page to start a download</span>
            </Link>
          </div>
