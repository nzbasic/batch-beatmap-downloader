...
import "./app/ipc/main";
import { getDownloadsStatus, pauseDownloads } from "./app/download/downloads";
import { setDownloadStatus } from "./app/download/settings";
...
const createWindow = (): void => {
  ...
  window.on('close', (e) => {
    e.preventDefault();
    pauseDownloads();
    const status = getDownloadsStatus()
    const save = status.filter(({ all, completed, failed, skipped }) => {
      const remaining = all.length - completed.length - failed.length - skipped.length;
      return remaining > 0;
    })

    Promise.all(save.map(i => setDownloadStatus(i.id, i))).then(() => {
      app.quit();
    })
  })
